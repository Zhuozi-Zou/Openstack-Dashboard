...
export function exportXML (json, canvas, createFile = true) {
  ...
  canvas.signalDefs.forEach(s => {
    signals += `${tab(2)}<signal id="${s.id}" name="${s.name}" flowable:scope="${s.scope}" />\n`
  })
  let messages = ''
  canvas.messageDefs.forEach(s => {
    messages += `${tab(2)}<message id="${s.id}" name="${s.name}" />\n`
  })
  let BPMNShape = ''
  let BPMNEdge = ''
  ...
  json.nodes.forEach(node => {
    BPMNShape += `${tab(6)}<bpmndi:BPMNShape bpmnElement="${node.id}" id="BPMNShape_${node.id}">\n` +
      `${tab(8)}<omgdc:Bounds width="${node.size[0]}" height="${node.size[1]}" x="${node.x}" y="${node.y}" />\n` +
      `${tab(6)}</bpmndi:BPMNShape>\n`
    switch (node.clazz) {
      case 'start':
        processXML += `${tab(4)}<startEvent id="${node.id}" />\n`
        break
      case 'end':
        processXML += `${tab(4)}<endEvent id="${node.id}" />\n`
        break
      case 'userTask': {
        let assignments = ''
      ...
      case 'javaTask': {
        ...
        processXML += `${tab(4)}<serviceTask id="${node.id}" name="${node.label}" ${javaClass} />\n`
        break
      }
      case 'scriptTask': {
        let script = ''
      ...
      case 'receiveTask':
        processXML += `${tab(4)}<receiveTask id="${node.id}" name="${node.label}" />\n`
        break
      case 'mailTask': {
      ...
      case 'signalStart': {
        const signal = `${tab(6)}<signalEventDefinition signalRef="${node.signal}" />\n`
      ...
      case 'messageStart': {
        const message = `${tab(6)}<messageEventDefinition messageRef="${node.message}" />\n`
      ...
      case 'gateway':
        processXML += `${tab(4)}<exclusiveGateway id="${node.id}" name="${node.label}" />\n`
        break
      case 'exclusiveGateway':
        processXML += `${tab(4)}<exclusiveGateway id="${node.id}" name="${node.label}" />\n`
        break
      case 'parallelGateway':
        processXML += `${tab(4)}<parallelGateway id="${node.id}" name="${node.label}" />\n`
        break
      case 'inclusiveGateway':
        processXML += `${tab(4)}<inclusiveGateway id="${node.id}" name="${node.label}" />\n`
        break
      default:
        break
    }
  ...
  json.edges.forEach(edge => {
    BPMNEdge += `${tab(6)}<bpmndi:BPMNEdge bpmnElement="${edge.source}_${edge.sourceAnchor}-${edge.target}_${edge.targetAnchor}" ` +
      ...
      `${tab(8)}<omgdi:waypoint x="${edge.startPoint.x}" y="${edge.startPoint.y}" />\n` +
      `${tab(8)}<omgdi:waypoint x="${edge.endPoint.x}" y="${edge.endPoint.y}" />\n` +
  ...
  const BPMNDiagram = `${tab(2)}<bpmndi:BPMNDiagram id="BPMNDiagram_${id}">\n` +
...
export function exportImg (canvasPanel, filename, createFile = true) {
  ...
  const canvas = canvasPanel.querySelector('canvas')
